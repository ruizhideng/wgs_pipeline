...
ls *.sorted.markdup.bam | while read id; do
    ${platypus} callVariants --bamFiles=$id --refFile=${platypus_reference} --output=${work_dir}/result/platypus/`basename $id .sorted.markdup.bam`.vcf --logFileName=`basename $id .sorted.markdup.bam`.platypus_log_file.txt --minPosterior=5
done
