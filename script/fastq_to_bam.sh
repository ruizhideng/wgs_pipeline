...
cat ${work_dir}/result/bwa/config | while read id; do 
    arr=(${id}); fq1=${arr[0]}; fq2=${arr[1]}; tmp=`basename ${arr[0]}`;sample=`echo ${tmp%%_*}`; bwa mem -t 2 -R "@RG\tID:$sample\tSM:$sample\tLB:WGS\tPL:Illumina" $reference $fq1 $fq2 | samtools view -Sb - > ${work_dir}/result/bwa/$sample.bam
done
