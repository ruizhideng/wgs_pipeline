...
fastqc ${work_dir}/test_data/*gz -o ${work_dir}/result/qc
...
ls ${work_dir}/test_data/*1.fq.gz > ./1;cat 1
ls ${work_dir}/test_data/*2.fq.gz > ./2;cat 2
...
ls *.bam | while read id; do samtools sort -@ 4 -m 4G -O bam -o ${work_dir}/result/bwa/`basename $id .bam`.sorted.bam $id;done
